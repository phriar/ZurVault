...
| `long-box.html` | Cross-collection "cheapest first" browse across every tracked collection at once, with a searchable min/max SOL price range and name search. Grid/List toggle. |
...
| `site-nav.js` | Shared top banner (injected via `<script>`). Primary nav: Listings, Characters, Collections, Artists, Rarity, Long Box, Spotlight, How To, Slideshow — `discover.html`, `comics.html`, and `candy-watcher.html` still work at their direct URLs but aren't in the banner. |
...
Browser ──> GitHub Pages (static HTML/CSS/JS, no build)
              ...
              │  artist.html, long-box.html, rarity.html            ├─> Cloudflare Worker: GET /v2/dc-summary
