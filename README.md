...
| `site-nav.js` | Shared top banner (injected via `<script>`). Primary nav: Listings, Characters, Collections, Artists, Long Box, Spotlight, How To, Slideshow — `discover.html`, `comics.html`, and `candy-watcher.html` still work at their direct URLs but aren't in the banner. |
...
- **Primary nav** (`site-nav.js`): Listings, Characters, Collections, Artists, Rarity, Long Box, Spotlight, How To, Slideshow — `discover.html`, `comics.html`, and `candy-watcher.html` are deliberately unlinked (not deleted, still fully functional at their direct URLs), as secondary/power-user tools outside the main collector experience.
