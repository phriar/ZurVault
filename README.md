...
| File | What it does |
|---|---|
| `index.html` | Landing page. Neon hero (See Latest Drops / Browse the Back Issue Bin), then a Magic-Eden-vs-OpenSea liquidity comparison widget (floor/24h volume/24h sales via the Worker's `GET /v2/liquidity`), then the live "For Sale" and "Recent Sales (7d)" feed across every tracked DC sub-collection. Grid/List view toggle. |
...
| `dashboard.html` | Added 2026-08-15, promoted to the primary nav 2026-08-22 once its price-history data had a few days to build up. Cross-collection floor-price trend dashboard — one sparkline row per tracked collection, via the Worker's `GET /v2/dc-history`. Magic Eden only (no Tensor integration). Three tabs: **All** (sorted by biggest mover, pending collections sink to the bottom), **Biggest Gainers**, and **Biggest Losers** (the latter two exclude collections still gathering history entirely, rather than sinking them — a pending collection is neither a gainer nor a loser). History can't be backfilled (Magic Eden's API has no historical floor-price data) and is written once per UTC day, so expect one flat point per collection for the first day or two, real %-change readings from day 3, and a genuine ~7-day "biggest movers" signal only after about a week — see "Things to remember." |
...
| `scout/index.html` | **AI comic-search POC, not linked from site nav** — added 2026-08-25, reworked 2026-08-28, see `SCOUT-PLAN.md`. A collector types a natural-language request; the page just posts it (plus small curated-notes maps built from `character-map.js`/`collections-map.js`) to the Worker's `POST /v2/scout`, where Claude runs a real agentic tool-use loop — `search_listings` (full live inventory, filtered by terms Claude picks itself, no hand-curated character list involved) and `get_price_context` (a series' recent floor/sales history, the actual "good deal" signal) — before answering. Results link back into `collection.html`/`long-box.html`, never out to a chat UI. Lives one directory deep (for the clean `/scout` URL), which is why `site-nav.js`'s links became root-relative — see that file's own comment. |
| `site-nav.js` | Shared top banner (injected via `<script>`). Primary nav (as of 2026-08-22): Listings, Characters, Artists, Back Issue Bin, Dollar Bin, Packs, Dashboard, How To — the "Characters" entry deep-links to `collections.html?tab=characters` (still the `collections` nav id, shared with `collection.html`). `discover.html`, `comics.html`, `candy-watcher.html`, `click-stats.html`, `grails.html`, and `spotlight.html` still work at their direct URLs (and via cross-links from other pages) but aren't in the banner. |
| `click-stats.html` | Added pre-2026-08-16, expanded that date and 2026-08-18. Outbound-click and conversion analytics — every click on a Magic Eden listing/sale card, individually, via the Worker's `GET /v2/click-stats`. Daily/hourly/day-of-week activity charts, a weekly click-to-possible-sale conversion chart, a Top Clicked Listings leaderboard (cover art enriched client-side from `/v2/dc-summary`), a per-collection conversion-rate breakdown, and a Possible-Sale Volume block (possible sales, rate, SOL volume, estimated USD volume via a live Coinbase spot price) — the "buyer activity flowing through the ecosystem" figure. A "Tracking since &lt;date&gt;" line sits under the headline counts so the 90-day/12-week window labels don't visually overstate how much history has actually accumulated yet. Written to double as a real report (not just an internal debug page) — explicitly scoped to click/conversion data only; session duration, page views, and visitor counts aren't instrumented anywhere on this site. **Not in the primary nav** — same unlinked-but-fully-functional pattern as `discover.html`/`candy-watcher.html`. |
...
Cloudflare Worker (zurvault-proxy)
  ...
  │                 + /v2/liquidity (single `liquidity-stats` KV entry,
  │                 same 30s edge-cache shape — Magic Eden vs OpenSea
  │                 floor/24h-volume/24h-sales for index.html's widget)
  └─ scheduled()  — Cron Trigger, every 20 min: loops DC_COLLECTIONS in small
                     ...
                     Also runs refreshLiquidityStats(env) in parallel (own
                     waitUntil, independent of the above): derives Magic
                     Eden's floor/volume/sales from the same merged KV data
                     and fetches OpenSea's candy-dc collection stats, writing
                     both into the single liquidity-stats KV entry above.
```
...
`/v2/scout` also rate-limits at 20 requests per IP per hour (Worker-side, via `DC_CACHE` — see `SCOUT-PLAN.md`'s Worker section) as a backstop against one IP running up requests, independent of the spend limit above. Each request can now run up to a few real Anthropic calls (a tool-use loop, not one forced call — see `SCOUT-PLAN.md`'s 2026-08-28 revision), so this is lower than an earlier testing-only value to keep per-IP spend exposure bounded.

### OpenSea API key
`OPENSEA_API_KEY` must be set as an encrypted Worker Secret (same dashboard path as `ANTHROPIC_API_KEY`: Cloudflare dashboard → the Worker → Settings → Variables → "Encrypt") for `refreshLiquidityStats()`'s OpenSea call (index.html's Magic-Eden-vs-OpenSea widget, served via `GET /v2/liquidity`) to work. **Get a real, persistent key from `opensea.io/settings/developer` (requires signing in) — not the self-issued "instant" key** (`POST https://api.opensea.io/api/v2/auth/keys`, no signup needed) used to prototype this feature, which expires after 7 days and is fine for a one-off manual check but not for a cron running indefinitely. Like every other Worker change in this repo, **it won't go live until `me-proxy-worker.js` is hand-pasted into the dashboard again**. Without the secret set, `refreshLiquidityStats()` just logs and skips every cron cycle, and `/v2/liquidity` keeps returning `{"notReady":true}` — nothing else on the site is affected.
...
> **⏳ Interim state: the liquidity widget is running on a temporary 7-day OpenSea key, not a persistent one.** Added 2026-09-01. `opensea.io/settings/developer`'s self-service "Create API Key" flow requires verifying an EVM (Ethereum-family) wallet, and the OpenSea account used here only has a Solana wallet linked — that blocked getting a real persistent key at rollout time. Deployed instead with a self-issued "instant" key (`POST https://api.opensea.io/api/v2/auth/keys`, no signup/wallet needed, documented in the "OpenSea API key" section above) that **expires 2026-09-08 ~11:05 UTC**.
>
> **If it lapses before a decision gets made**: nothing breaks. `refreshLiquidityStats()` just starts failing its OpenSea fetch every cron cycle (logged via `console.error`, never thrown), the `liquidity-stats` KV entry stops refreshing and expires on its own TTL (`LIQUIDITY_TTL_SECONDS`, 40 min), and `/v2/liquidity` reverts to `{"notReady":true}` — the widget shows "Still gathering data — check back shortly." indefinitely, same as it would before any key ever existed. No user-facing error, nothing else on the site affected.
>
> **To keep the feature**: link an EVM wallet to the OpenSea account (Settings → Wallets — doesn't need funds, just something to sign a verification message with), then get a real persistent key from `opensea.io/settings/developer` and paste it into the `OPENSEA_API_KEY` Worker secret, replacing the instant one. Cheaper stopgap: just re-issue another instant key (`curl -X POST https://api.opensea.io/api/v2/auth/keys`) and re-paste it — repeat weekly for as long as that's tolerable.
>
> **To roll it back instead**: no code revert needed — just leave `OPENSEA_API_KEY` unset (delete the secret, or let the key lapse and don't replace it). The widget already degrades to its permanent "gathering data" state with the secret missing, which is functionally identical to the feature not existing from a visitor's perspective. Only revert the `me-proxy-worker.js`/`index.html`/`README.md` changes themselves if the widget's markup needs to be fully gone from the page, not just quietly empty. This note should be deleted once a persistent key is in place (or the feature is rolled back) — whichever happens first.
...
- **`site-nav.js`'s links are root-relative (`/index.html`, not `index.html`), added 2026-08-25 for `scout/index.html`.** Scout lives one directory deep — a document-relative link would resolve `index.html` against `scout/`'s own location (i.e. back to Scout itself) instead of the real homepage. Root-relative fixes that and is a no-op for every page at the root, so nothing else changed behavior. Keep this in mind if a future page ever moves into its own subdirectory the same way.
- **Primary nav** (`site-nav.js`, as of 2026-09-01): Listings, Characters, Artists, Back Issue Bin, Dollar Bin, Packs, Dashboard, How To, ZurCovers ↗. The last entry (added 2026-09-01) is the first `PAGES` entry that isn't a page in this repo — a plain outbound link to the sibling zurcovers.com product (opens in a new tab, `external: true` in `PAGES`), added after enough manual tab-switching between the two sites that a persistent link made sense. `long-box.html` relabeled from "Long Box" to "Back Issue Bin" in the nav and on-page copy 2026-08-22 — URL, nav `id` (`long-box`), and filename all unchanged, this is a display-name-only rename — to avoid brand collision with ZurCovers' own "Long Box" feature (its wallet-collection shelf view, a completely different concept: what you already own vs. what's for sale here). `discover.html`, `comics.html`, `candy-watcher.html`, and `click-stats.html` are secondary/power-user tools or in-progress pages that were never linked in the first place. `grails.html` and `spotlight.html` joined the unlinked group 2026-08-19 by user request — not deleted, still fully functional at their direct URLs and via cross-links from other pages (e.g. `index.html`'s spotlight teaser), just removed from the persistent top banner. `dashboard.html` moved the other direction 2026-08-22, added to the primary nav once its price-history data had a few days to build up. `collections.html` unlisted 2026-08-19 alongside `grails.html`/`spotlight.html`, then relisted 2026-08-22 — labeled "Characters", deep-linking straight to the Characters tab (`?tab=characters`) rather than the Series tab, since browsing by character (pick one, see every comic featuring them across collections) is the entry point that matters now that character-based weekly "quests" are on the roadmap. `characters.html`/`character.html` were retired outright (not just unlisted) — character browsing lives inside `collections.html`/`collection.html`; that retirement wasn't reversed, only the nav link changed.
...
**Is the Magic-Eden-vs-OpenSea liquidity widget (`index.html`) actually running?**
```
curl -s https://zurvault-proxy.stholt.workers.dev/v2/liquidity | head -c 500
```
- `{"notReady":true}` → either no `collection:*` KV entries exist yet (same cause as `/v2/dc-summary`'s `notReady`), or `OPENSEA_API_KEY` isn't configured — check the Worker's execution logs for `refreshLiquidityStats: OPENSEA_API_KEY not configured` vs. `refreshLiquidityStats failed:` to tell which.
- Real data but `"openSea":{"floorPriceSol":null,...}` with everything else populated → most likely the SOL-currency guard tripped (OpenSea's current cheapest `candy-dc` listing isn't SOL-denominated), not a bug — check the Worker logs for `refreshLiquidityStats failed:` first to rule out an actual fetch failure before assuming that.
- `magicEden.floorPriceSol` should never be near-zero (glitched listings below `SCOUT_MIN_SANE_PRICE_SOL` are filtered out) — if it looks implausibly low, cross-check against a manual scan of `/v2/dc-summary`'s listings for that sub-collection.
...
**Is Scout (`/v2/scout`) actually configured?**
```
curl -s -X POST https://zurvault-proxy.stholt.workers.dev/v2/scout \
  -H "Content-Type: application/json" \
  -d '{"query":"test"}'
```
`{"error":"scout_unavailable"}` (HTTP 503) means the deployed Worker doesn't have `ANTHROPIC_API_KEY` set yet — add it as a Worker Secret (see Configuration above), returned before any Anthropic call is made so this half of the check costs nothing either way. A real `{"summary":...,"recommendations":[...],"toolTrace":[...]}` response means the key is live and Scout actually searched — unlike the old design this runs Claude's real tool-use loop against live data (a few real, cheap Haiku calls), not a guaranteed-empty check.
