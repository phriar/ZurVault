...
| `site-nav.js` | Shared top banner (injected via `<script>`). Primary nav: Listings, Grails, Collections, Artists, Long Box, Spotlight, How To, Slideshow — `discover.html`, `comics.html`, and `candy-watcher.html` still work at their direct URLs but aren't in the banner. |
...
### Helius API key
`discover.html` and `candy-watcher.html` prompt the user to paste their own key (kept session-only, never persisted or hardcoded). The live `slideshow.html` is a static placeholder and makes no RPC calls at all.
...
- **`slideshow.html` is a static placeholder ("Coming Back Soon"), by design.** Google Safe Browsing flagged the page under "Deceptive pages" — a visible wallet-address input sitting next to a "Connect Phantom" button structurally resembles wallet-drainer phishing kits to their classifier, even though the feature was 100% read-only. The old implementation (Phantom connect, `?wallet=<public-address>` kiosk mode, `localStorage`-persisted address, Helius lookups) briefly lived on unlinked at `slideshow-legacy.html`; after a second, unrelated Safe Browsing flag on 2026-08-09, it was removed from the repo entirely (not just unlinked) rather than left reachable at its own direct URL, and archived locally outside the repo instead. Requesting a review in Google Search Console (Security Issues) is required after any fix like this — the flag never clears on its own just because the page changed. **Don't recreate this feature without addressing the underlying UI pattern first.**
