...
| File | What it does |
|---|---|
| `index.html` | Landing page. Neon hero (See Latest Drops / Browse the Back Issue Bin) above the live "For Sale" and "Recent Sales (7d)" feed across every tracked DC sub-collection. Grid/List view toggle. |
...
| `scout/index.html` | **AI comic-search POC, not linked from site nav** — added 2026-08-25, see `SCOUT-PLAN.md`. A collector types a natural-language request; the page filters live `/v2/dc-summary` listings down to ~20-40 candidates client-side, posts them plus the query to the Worker's `POST /v2/scout`, and Claude ranks/explains a handful using its own knowledge of DC publication history — no pre-researched key-issue database required. Results link back into `collection.html`/`long-box.html`, never out to a chat UI. Lives one directory deep (for the clean `/scout` URL), which is why `site-nav.js`'s links became root-relative — see that file's own comment. |
...
curl -s -X POST https://zurvault-proxy.stholt.workers.dev/v2/scout \
  ...
  -d '{"query":"test","candidates":[{"mintAddress":"test123"}]}'
```
`{"error":"scout_unavailable"}` (HTTP 503) means the deployed Worker doesn't have `ANTHROPIC_API_KEY` set yet — add it as a Worker Secret (see Configuration above). A real `{"summary":...,"recommendations":[]}` response (empty recommendations is expected here — `"test123"` isn't a real mint, so it gets validated out) means the key is live and working.
